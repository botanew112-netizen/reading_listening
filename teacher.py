...
with tab7:
    st.markdown("### 📖 Reading — Оқушы нәтижелері")

    if st.button("🔄 Жаңарту", key="reading_ref"):
        st.cache_data.clear()
        st.rerun()

    @st.cache_data(ttl=60)
    def fetch_reading_attempts() -> list:
        try:
            res = (get_supabase()
                   .table("test_attempts")
                   .select("*, tests(title)")
    ...
    attempts   = fetch_reading_attempts()

    if not attempts:
        st.info("Әлі ешкім Reading тест тапсырмаған.")
    else:
        submitted = [a for a in attempts if a.get("submitted_at")]
        total_att = len(submitted)
        avg_band  = (
            round(sum(float(a.get("band") or 0) for a in submitted) / total_att, 1)
            if total_att else 0
        )
        unique_st = len(set(a.get("student_name","") for a in submitted))

        m1, m2, m3 = st.columns(3)
        m1.metric("Барлық тапсырма", total_att)
        m2.metric("Оқушы саны",      unique_st)
        m3.metric("Орташа Band",     avg_band)
        st.markdown("---")

        names_with_attempts = sorted(set(
            a.get("student_name","") for a in submitted if a.get("student_name")
        ))

        sel = st.selectbox(
            "Оқушыны таңдаңыз:",
            ["— Барлығы —"] + names_with_attempts,
            key="reading_student_sel"
        )

        if sel == "— Барлығы —":
            st.markdown("#### Барлық оқушылар (соңғы тапсырма)")
            ...
            rows = sorted(latest.values(),
                          key=lambda x: float(x.get("band") or 0), reverse=True)

            for i, a in enumerate(rows, 1):
                name  = a.get("student_name", "—")
                score = a.get("score", "—")
                band  = a.get("band",  "—")
                title = (a.get("tests") or {}).get("title", "—")
                date  = (a.get("submitted_at") or "")[:16].replace("T", " ")

                try:
                    b = float(band)
                    if b >= 7:   bc, bb = "#27500A", "#EAF3DE"
                    elif b >= 6: bc, bb = "#854F0B", "#FAEEDA"
                    else:        bc, bb = "#A32D2D", "#FCEBEB"
                except Exception:
                    bc, bb = "#555", "#F4F4F4"
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;
                  ...
                  <div style="font-size:14px;font-weight:500;min-width:160px;">{i}. {name}</div>
                  <div style="font-size:12px;color:#666;flex:1;padding:0 12px;">{title}</div>
                  <div style="font-size:12px;color:#888;min-width:120px;">{date}</div>
                  <div style="background:#F4F4F4;border-radius:6px;padding:4px 10px;
                              font-size:13px;min-width:60px;text-align:center;">{score}/40</div>
                  <div style="background:{bb};color:{bc};border-radius:6px;
                              padding:4px 12px;font-size:15px;font-weight:700;
                              min-width:70px;text-align:center;margin-left:8px;">Band {band}</div>
                </div>""", unsafe_allow_html=True)

        else:
            st.markdown(f"#### 👤 {sel} — тарихы")
            student_attempts = [a for a in submitted if a.get("student_name") == sel]

            if not student_attempts:
                st.info("Бұл оқушының тапсырмасы жоқ.")
            else:
                for a in student_attempts:
                    ...
                    with st.expander(f"📝 {title}  ·  {score}/40  ·  Band {band}  ·  {date}"):
                        c1, c2 = st.columns(2)
                        c1.markdown(f"""
                        <div style="background:{bb};border-radius:10px;padding:16px;text-align:center;">
                          <p style="margin:0 0 4px;font-size:12px;color:{bc};">Band Score</p>
                          <p style="margin:0;font-size:32px;font-weight:700;color:{bc};">{band}</p>
                        </div>""", unsafe_allow_html=True)
                        c2.markdown(f"""
                        <div style="background:#F4F4F4;border-radius:10px;padding:16px;text-align:center;">
                          <p style="margin:0 0 4px;font-size:12px;color:#555;">Дұрыс жауап</p>
                          <p style="margin:0;font-size:32px;font-weight:700;">{score}/40</p>
                        </div>""", unsafe_allow_html=True)

                        if answers:
                            st.markdown("**Жауаптар:**")
                            ans_html = "<div style='display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;'>"
